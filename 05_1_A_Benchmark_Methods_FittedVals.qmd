...
$$
\begin{equation}
\hat{y}_{T+h|T} = y_{T} + h  \underbrace{\frac{\sum_{t=2}^T\Delta_i y_t}{T-1}}_{\substack{\text{average change of } y \\ \text{in the historical data}}} = y_{T} + \frac{h}{T-1}\sum_{t=2}^T (y_{t}-y_{t-1}) = y_{T} + h \left( \frac{y_{T} -y_{1}}{T-1}\right)
\end{equation}
$$
