...
The following figure will serve to clarify the difference between the concepts of **fitted values** and **forecasts**. This figure **summarizes the forecasting task** and is **very relevant for both this and the second part of the course**:
...
* The blue dots represent the **fitted values**, values returned by the model we have fitted in the training region.
...
* Residuals in the transformed domain (**innovation residuals**): $w_t - \hat{w_t}$

By reversing the transformation (backtransforming) we would obtain:

* Fitted values in the original domain: $\hat{y_t} = e^{\hat{w}_t}$
* Residuals in the original domain (**residuals**): $y_t - \hat{y_t}$
...
# TODO: incluir aquí, por separado, el modelo al season adjust y al seasonal_c
# y hace runa figura donde se muestren estos junto con los forecast totales.

# Ahora mismo lo hago en clase y es suficiente, pero quizá debería hacerlo aquí.
```
...
In the `fpp3` library, we attain all of the above using the function `decomposition_model()`. Below a summary image of what this function attains:

![](figs/decomposition_model_function.png){fig-align="center"}
