...
After analyzing the graph, we can conclude that although the total cost has been raising and dropping constanlty throuought the years, in a bigger picture the costs rose year over year.
...
After visualizing the seasonalities and subseries graphics we can confirm that costs increase every year. However, it is important to remark that the price varies during the year with Febreaury being the month with lower costs and January/December being the months with higher costs.
...
After conducting the ACF we could recognize on first sight the peak at lag_m = 12. This confirmed the distribution of data into months and the lagged plot proved it.
...
# Your code goes here
total_private %>%
  autoplot() + scale_x_yearmonth(breaks = "10 years",
                      minor_breaks = "1 year")
```
...
The amount of employees remains constant throughought the year. However, there a constant increase of employer year over year. This can be easily confirmed by having a look at the subseries plot too.
...
Both plots shows the strong correlation of the data. the ACF shows how correlation peaks in 1 or almost one and then slightly decrease.
...
We need to re index for the interval to be determined by the greater common divisor.
...
```{r}
major <- seq(0, max(damzn$trading_day), by = 20)
minor <- seq(0, max(damzn$trading_day), by = 10)


damzn %>%
  autoplot(Close) +
  scale_x_continuous(breaks = major,
                     minor_breaks = minor)
...
More than 95% of the spikes lie within the bounds so we can consider it to be a white noice. However, TWO **large spikes** are outside these bounds so therefore it could also not be a white noise. This should be further analyzed for false positives.
...
If we focus on the spikes we can clearly determine that it is not a white noise. "*If one or more **large spikes** are outside these bounds or if substantially more than 5% of the spikes are outside these bounds, then the time series is probably not white noise.*"
