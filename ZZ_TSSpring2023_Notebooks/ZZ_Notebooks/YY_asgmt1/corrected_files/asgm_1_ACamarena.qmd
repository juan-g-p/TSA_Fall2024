...
```{r}
costs.PBS_h02 %>%
  gg_season(tcost, labels = "both") + # Labels -> "both", "right", "left"
  labs(y = "$ (millions)",
       title = "Seasonal plot: Australia Monthly Medicare")
```
...
You can see how by the years there's a trend in the price going up because of the economy, and also you can see how from July to December there's a spike going up and in January the spike takes a downfall.
...
costs.PBS_h02 %>%
  gg_subseries(tcost, labels = "both") + # Labels -> "both", "right", "left"
  labs(y = "$ (millions)",
       title = "Seasonal Subseries plot: Australia Monthly Medicare")
...
We can see how still theres' a trend going up from February to December and then start doing downfall onwards until january. Is the same pattern along the years.
...
costs.PBS_h02%>%
  gg_lag(y = tcost, geom = "point", lags = 1:12)
```
...
We can see a strong yearly seasonality of the data at lag =12.
...
total_private %>% 
  ...
  scale_x_yearmonth(breaks = "10 years",
                    minor_breaks = "1 year") +
  xlab("Year") + 
  ylab("Monthly US Employed in Thousands")
```
...
total_private %>%
  gg_subseries(Employed, labels = "both") + # Labels -> "both", "right", "left"
  labs(y = "$ (Thousands)",
       title = "Seasonal Subseries plot: US Employment")
```
...
Along the years we can see that the number of employees doesn't change or slightly changes, but in the time plot appeared like an increasing number of employees through the year.
...
Both plots shows a strong correlation of the data.
...
damzn %>%
  autoplot(Close) +
  scale_x_continuous(breaks = seq(0, max(damzn$trading_day), 20),
                     minor_breaks = seq(0, max(damzn$trading_day), 10))
damzn
...
```{r}
Closing_Prices = autoplot(ACF(damzn, Close, lag_max = 100))
Closing_Prices

Diff_Prices = autoplot(ACF(damzn, diff, lag_max = 100))
Diff_Prices
```
...
If there's a 2% of spikes out of range we can infer that the difference in the stock look like white noise.
...
There's various spikes that are out of range more than 2% so we can say that is not white noise.
