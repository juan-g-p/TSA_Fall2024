...
By analyzing this time series, it is possible to identify a positive relationship. Each year, costs start being low and then increase throughout the year.
...
# Your code goes here
PBS_h02_month %>%
  gg_season(monthly_cost, labels = "both") +
  labs(y = "Cost in dollars",
       title = "Cost Seasonalities")

PBS_h02_month %>%
  gg_subseries(monthly_cost, labels = "both") + 
  labs(y = "Cost in dollars",
...
There is a decrease in costs in February, and then they starts to increment as the year advances. Moreover, throughout the years the costs of each month increases.
...
PBS_h02_month %>%
  gg_lag(y = monthly_cost, geom = "point", lags = 1:12)
```
...
After 12 lags, it is possible to identify a yearly seasonality. The ACF shows a spike once every 12 months.
...
total_private %>%
  ...
  labs(y = "Employed",
       title = "Employment Levels_Different Seasons") + 
...
In the time series, a positive relationship can be identified. Each year the employed increase, but within each year these numbers are almost constant.
...
From the lags, we can see a significant positive correlation between the plots and variables. Also, since the ACF is decreasing though time (not much), the relationship becomes weaker.
...
Since we are talking about stock prices, we only take into consideration the trading days, not the weekends. That is why, we need to re-index the data.
...
It does like white noise, there is not any trend that seems to be affecting the data. There is random variability.
...
Since in the original series we can identify a significant correlation between the lags and the variables, we can say that it does not like the white noise.
