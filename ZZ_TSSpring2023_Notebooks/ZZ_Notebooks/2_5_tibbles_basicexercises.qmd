...
1.  Select the subset of the babynames dataset that contains data between 1990 and 2010 about female babies:
...
2.  Select the subset of the babynames dataset that contains data between 1990 and 2010 regarding babies named either Mary, Sarah or Carrie:

```{r, include=params$print_sol}
names_list <- c("Mary", "Sarah", "Carrie")
...
3.  Filter the flights dataset to contain only the flights that took place in June, July, August and September. Keep only the subset of flights that were delayed. Store the result in a variable called `res`
...
4.  Take the result of 3 and select only the subset of flights that have a plane tail number (variable `tailnum`) that starts with N. For this you will need to use the function `substr()`. Research online how the function works.
...
1.  Take the flights dataset and select only the first 3 columns

```{r, include=params$print_sol}
flights %>% select(1:3)
```

2.  Take the flights dataset and select only the columns between `year` and `dep_delay`

```{r, include=params$print_sol}
flights %>% select(year:dep_delay)
```

3.  Take the flights dataset and select all the columns except those between `year` and `dep_delay`:

```{r, include=params$print_sol}
flights %>% select(-(year:dep_delay))
```

4.  Take the flights dataset, place the variable `tailnum` at the beginning of the dataset, drop the variable `carrier` and keep the rest of the columns in their original order:
...
1.  Use `mutate()` to create two new columns in the flights dataset:
...
flights %>% 
  mutate(
    ...
    hour_ind = ifelse(arr_delay / 60 < 1, FALSE, TRUE)
  ) %>% 
  select(date, hour_ind, everything())
```

2.  Use `mutate()` and `substr()` to create a new column with the initial letter of the name of every entry in the dataset `babynames`.
...
1.  Use `group_by()` + `summarize()` to get the `mean()` and `median()` value of `arr_delay` each month. You will need to specify a correct value for the argument `na.rm` within `arr_delay()`
...
1.  Use `group_by()`, `filter()` and `ungroup()` to get a list of the most unpopular male and female `babynames` each year
...
1.  Use `group_by()`, `mutate()` and `ungroup()`

Use the function `dense_rank()` in combination with `group_by()`, `mutate()` and `ungroup()` to create a rank of flights within the same day based upon their associated `arr_delay`. Research online or on the help how `dense_rank()` works.

At the same time, use the function `row_number()` to assign an integer number to each row of the same daily partitions (biggest arrival delay gets the smallest `row_number`)
