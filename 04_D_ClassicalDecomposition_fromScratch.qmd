...
$$
\begin{align*}
\text{Condition we want to attain}: && \sum_{i=1}^{m}S_i = \sum_{i=1}^{m}(S_{unadj\text{ i}} + x) = m \\
\text{Developing the sums: } && \sum_{i=1}^m{S_i} = \sum_{i=1}^mS_{unadj\text{ i}} + \sum_{i=1}^mx = a + m \cdot x = m \\
\text{Hence, the resulting correction x: } && x = \frac{m - a}{m} = 1-\frac{a}{m}
\end{align*}
$$

$$
\sum_{i=1}^{m}(S_{unadj\text{ i}} + x) \underset{\uparrow \\ \text{imposed}}{=} m
$$
...
res <- res %>%
  # Correction so that the sum of the seasoan components is 0
  mutate(seasonal_c = S_unadj + correction)
...
manual_decomposition %>% 
  ...
  mutate(
    seasonal_c = S_unadj + 1 - (sum(S_unadj) / 4)
  ) %>% 
  select(n_quarter, seasonal_c) %>% 
  as_tibble() %>% 
  right_join(., manual_decomposition %>% select(-seasonal_c) %>% as_tibble(), by="n_quarter") %>% 
  as_tsibble(index=Quarter)

all.equal(res2$seasonal_c, classical_dec$seasonal)

sum(classical_dec$seasonal[1:4])
sum(res2$seasonal_c[1:4])


res$seasonal_algo = classical_dec$seasonal[3:6]

res$diff_man = res$S_unadj - res$seasonal_c
res$diff_algo = res$S_unadj - res$seasonal_algo

res$diffs_ratio = res$diff_algo / res$diff_man
...
res %>% 
  mutate(
    a = sum(S_unadj),
    correction1 = 1 - a/4,
    correction2 = correction1 * S_unadj * (m/a),
    seasonal_c2 = S_unadj + correction2
  )
