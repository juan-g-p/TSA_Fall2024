...
* $\hat{y}_{t+h|t}$ is the forecast at $t+h$ given everything occurred up to $t$.
* $\alpha$ is the smoothing parameter of the model.
* $l_t$ is the level of the time series at time $t$. 
    * Since fitted values are one-step ahead forecasts on the training data, if we set $h=1$ on the forecast equation we see that $l_t$ is actually $\hat{y}_{t+1|t}$, that is, the fitted value at time $t+1$.
    * The value of the level at $t=T$ (at the last point where data is available) is therefore $\hat{y}_{T+1|T}$. 
        * Since SES produces flat forecasts by design, this is equal to $\hat{y}_{T+h|T}$, the flat (constant) forecast of SES.

**IMPORTANT:**

* setting $h=1$ gives the equations for the fitted values (one-step ahead forecasts on the training data).
* setting $t=T$ (last point for which data is available) gives the true forecasts beyond the trainig data.

The component form is more useful from a computational standpoint. For Simple Exponential Smoothing it might seem unnecessary, but it will be most convenient when more components are added in the trended and seasonal forms of exponential smoothing.

For simple exponential smoothing the component equations can be interpreted as follows:

-   **The forecast equation** shows that the forecast value at time $t+1$ is the estimated level at time $t$ (simply set $h=1$ in the forecast equation):

$$
\begin{align*}
\hat{y}_{t+1|t} = l_{t} && \text{(forecast equation for h=1)}
\end{align*}
$$

-   **The smoothing equation for the level** (a.k.a level equation) gives the estimated level of the series at time t as a **weighted average between the value of the series at time $t$ (that is, $y_t$) and the level of the time series at $t-1$ (that is, $l_{t-1}$)**. It is a weighted average because the coefficients of $y_t$ and $l_{t-1}$ add up to 1 ($\alpha + (1-\alpha) = 1$):
...
For Simple Exponential Smoothing this is a non-linear minimization problem that has no explicit solution. Therefore, it needs to be solved numerically, typically using iteration to choose the values of $\alpha$ and $l_0$ that minimize the SSE. The process could be summarized as follows:
