---
title: "05_6_Point_Forecast_Accuracy"
format: html
editor: source
params:
  print_sol: false
...
# Manual check 1: compute the MAE and RMSE manually for the drift model in the example above both for the training dataset and for the test dataset. Compare it to the output of the `accuracy()` function:
...
# Manual check 2: compute the MASE and RMSSE manually for the drift model in the example above. Compare it to the output of the `accuracy()` function
...
```{r, include=!params$print_examples}
# 1. Extract the residuals produced by the drift model from beer_fit 
# using augment(), filter() and pull()
...
```{r, include = params$print_examples}
# 1. Extract the forecasts produced by the drift model from 
# beer_fc using filter() and pull()
fc <- beer_fc %>% filter(.model == "Drift") %>% pull(.mean) %>% na.omit()
...
```
  ...
  * To what kind of forecast errors can we compare the residual errors?
