...
$$
The residuals are, by definition, the value of the time series minus the fitedd values. Therefore, replacing the formula obtained above, for the naive model the value of the residuals at a generic point in time t is:
...
n_obs_60 <- # Your code goes here. Compute the integer number covering 80% of the observations
...
### **5. Compute error metrics for each training dataset
...
**Option 2: computing the errors for each training datasets and using the formulas for MAE, RMSE and MAPE**. Follow the steps below:
...
We want these error metrics for each combination of:

- Forecast horizon (h)
- Model
- Training dataset (.id)

**At the end, check that both results match with `all.equal`.** Importantly, be sure to arrange both dataframes in the same order before making the comparison.
...
Produce two graphs, one for the absolute errors and one for the absolute percent errors contained in `error_metrics` (question 5.1) that are laid out as follows:

1. The x-axis grid only shows integer numbers and the x-axis and corresponds to the forecast horizon.
2. The y-axis corresponds to the values of the absolute error or the percent error.
3. Each graph has 8 lines, one line for each combination of model and training dataset (4 models and 2 training datasets in total).

**Use `ggplot` to produce these graphs.**

```{r}
# GARPH FOR ABSOLUTE ERRORS


```

```{r}
# GRAPH FOR ABSOLUTE PERCENTAGE ERRORS
...
* Does one of the models outperform the other? Are these results the same for both training datasets?
...
7.1 Take all the steps necessary to obtain the cross validated MAE, RMSE and MAPE for each combination of model and forecast horizon. The final table shall therefore have 3 x 10 rows (3 models and 30 forecast horizons) and, for each row, a column for MAE, another for RMSE and another for MAPE.
...
### **8. Produce the following graphs and answer the questions** (2.5 points) 

Produce two graphs. One for the absolute error and one for the percentage error.
...
* What is the difference between these graphs and the ones we generated in step 6? Answer briefly below.
...
### **9. Reflect on the results and present some conclusions** (0 point)
