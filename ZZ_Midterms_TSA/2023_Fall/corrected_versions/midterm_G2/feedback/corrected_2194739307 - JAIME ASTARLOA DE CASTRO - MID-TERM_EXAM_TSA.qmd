...
There is a clearly repeating pattern every 5 weeks that indicates weekly seasonality. With monthly data, the seasonal period wold be m = 12.
...
FEEDBACK: YOU WERE SUPPOSSED TO USE A MOVING AVERAGE!
...
dcmp_classic <- 
  scs_pedestrians %>% 
  model(
    classical = classical_decomposition(mean_val)
  ) %>% 
  components()
...
FEEDBACK: WHY ARE YOU DOING AN STL DECOMPOSITION? WHY??

FEEDBACK: CLEARLY, WHAT I SEE HERE SHOWS THAT YOU HAVE PAID NO ATTENTION AT ALL IN CLASS OR READ ANYTHING.

YOU ARE JUST COPY-PASTING CODE LIKE CRAZY

```{r}
## YOUR CODE GOES HERE
stl_default <- 
 scs_pedestrians %>%
  model(stl = STL(mean_val)) %>%
  components()
...
stl_adjust <- 
  scs_pedestrians %>%
  model(stl = STL(mean_val ~ trend(window = 5) + drift(window = 13) + snaive(window= 10))) %>%
  components()
...
FEEDBACK: WRONG FORECAST HORIZON AND WRONG MODELS. THIS IS JUST A COPY PASTE FROM THE MIDTERM WHERE YOU DO NOT UNDERSTAND ANYTHING
...
FEEDBACK: THE SAME GOES FOR THIS.
