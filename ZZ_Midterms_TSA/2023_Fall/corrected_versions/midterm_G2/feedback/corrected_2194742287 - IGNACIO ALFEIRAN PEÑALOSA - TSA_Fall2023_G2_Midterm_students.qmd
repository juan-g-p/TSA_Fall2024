...
Based on the time-plot, I would consider the seasonal period to be m = 1 week. This is because there is a repeating pattern of peaks  every 7 days, which suggests that the data is weekly.
...
geom_line(aes(y = `trend_class`), colour = "#D55E00") + geom_line(aes(y = `mean_val`), colour = "blue", linetype = "dashed") +
labs(y = "",
title = "Moving Average Scs pedestrians")
...
YOUR ANSWER GOES HERE (Max. 30 words)
 ...
 would pick the mean model to produce forecasts of up to 1 week ahead. The graph shows a relatively stable M.A  over time, with no clear trend or seasonality.
 
 FEEDBACK: THERE IS CLEAR SEASONALITY, YOU COULD ALSO PICK THE SEASONAL NAIVE. THE ONE THAT DOES NOT MAKE SENSE IS THE DRIFT.
...
```{r}
## YOUR CODE GOES HERE
ACF_original<- autoplot(ACF(scs_pedestrians, Count))
ACF_original
...
I wasnt able to plot one so I can not compare them :( 

FEEDBACK: I LIKED THE EMOJI :)
...
If the mean of the resiuduals will be 0, 

If the residuals means is 0 the mean model will be unbiased.

FEEDBACK... WHY DID YOU NOT CHECK THIS?
