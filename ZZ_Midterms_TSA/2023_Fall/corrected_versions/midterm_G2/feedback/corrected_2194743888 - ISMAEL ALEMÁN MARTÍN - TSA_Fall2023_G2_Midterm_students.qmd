...
there is a pattern repeated every week, therefore indicates weekly seasonality.
...
dcmp_classic %>%
  ...
  geom_line(aes(y = `mean_val`), colour = "#D55E00") +
    scale_x_yearweek(breaks = "5 weeks",
                      minor_breaks = "1 week") +
...
#Drift
fit <- scs_pedestrians %>% model(Drift = RW(Count ~ drift()))
...
fitted_vals %>% 
  ...
  geom_line(aes(y=.fitted), colour = "blue", linetype = "dashed")
#Naive
fit <- scs_pedestrians %>% model(SNaive = SNAIVE(Count ~ lag("week")))
fitted_vals <- fit %>% augment()
head(fitted_vals) 
fitted_vals %>% 
  filter(.model == "SNaive") %>%
