...
scs_pedestrians %>% autoplot()+
  ...
  labs(title = "Time plot with breaks")
...
The seasonal period I would consider is 7 days, as the data indicates a weekly cicle in the daily pedestrian count. Therefore m=7.
...
FEEDBACK: the MA is wrong. See solution
...
For forecasting up to 1 week ahead, the Seasonal Naive model would be the best model. This is due to the fact that the data has high weekly seasonality.
...
FEEDBACK: you only forecast one week ahead...

```{r}
fc = fit %>% forecast(h = 1, level = c(80, 95))

fc %>% filter(.model == 'snaive') %>% autoplot(scs_pedestrians) + labs(title = 'Forecast SES')
fc %>% filter(.model == 'mean') %>% autoplot(scs_pedestrians) + labs(title = 'Forecast Drift')
```
