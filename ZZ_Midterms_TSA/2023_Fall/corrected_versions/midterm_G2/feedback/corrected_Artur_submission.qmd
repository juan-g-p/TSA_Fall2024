...
FEEDBACK: COMPUTED MEAN INTSTEAD OF TOTAL COUNT
...
pedestrian1 <- 
  ...
  filter(Date >= ymd("2016-07-01") & Date<= ymd("2016-10-25")) %>% group_by(Sensor) %>%
index_by(day = Date) %>% summarise(daily_mean = mean(Count))
pedestrian1
...
p1 <- pedestrian1 %>% filter(Sensor == "Southern Cross Station") %>% 
ggplot(aes(x = day, y = daily_mean)) + 
  geom_line() + scale_x_yearweek(date_breaks = "5 week", minor_breaks = "1 week")
p2 <- pedestrian1 %>% filter(Sensor == "Bourke Street Mall (North)") %>% 
ggplot(aes(x = day, y = daily_mean)) + 
  geom_line() +scale_x_yearweek(date_breaks = "5 week", minor_breaks = "1 week")
p1
p2
#pedestrian1 %>% filter(Sensor == "Southern Cross Station") %>% geom_line(aes(y = Count), colour = "#D55E00")
...
feedback: this is the max, but not the day :)

```{r}
## YOUR CODE GOES HERE

f1 <- pedestrian1 %>% filter(Sensor == "Southern Cross Station") 
f2 <- pedestrian1 %>% filter(Sensor == "Bourke Street Mall (North)") 
...
The length of the seasonal period is one week, because we have a clear peak during the middle of every week which lowers heavely on both sides towards the start and end of the week. As the data is in days and a week has 7 days we should use 7
...
scs_pedestrians
scs_pedestrians %>%
  autoplot(Count) + geom_line(aes(y = trend_class), colour = "#D55E00") + geom_line(aes(y = mean_val), color = "red", linetype = "dashed")
...
------
The data has no trend but strong seasonality. The drift model is used for estimating the trend of a series so it is not of much use here. The mean model extrapolates the mean of the past data into the future so it would provide bad forecasts on the strongly oscilating data. The SNaive model can be used well to model seasonal components of data
...
The both plots are identical Because  the mean model only produces A line through the middle of the graph. The residuals are the same values as the original data only shifted by the mean. FEEDBACK: perfect

Because the residuals are exactly the same 
...
The model is not biased because it has a mean very close to 0

------

FEEDBACK: ARTUR, ONE OF THE BEST EXAMS. CONGRATS.