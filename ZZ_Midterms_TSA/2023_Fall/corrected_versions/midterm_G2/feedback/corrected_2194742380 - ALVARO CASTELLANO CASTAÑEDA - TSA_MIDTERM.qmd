...
FEEDBACK: WHY DO YOU LOAD SO MANY LIBRARIES. OMG... THIS CAN CAUSE TROUBLE.
```{r}
library(fpp3)
library(fpp3)
library(tidyverse)
...
scs_pedestrians  %>%
   autoplot(Count) + 
  scale_x_yearweek(date_breaks = "5 weeks", date_minor_breaks = "1 week") +
...
I would use the mean model to produce the 1 week ahead forecast, due to the fact that it is the most constant and hence would produce the best forecast for up to 1 week ahead.

FEEDBACK: IT IS A POSSIBLITY, BUT YOU ARE LEAVING THE SNAIVE, WHICH IS THE ONLY ONE WITH SEASONALITY AMONG THOSE PROPOSED.
...
The mean of the residuals is actually 0. **This is a good example of how a very bad model can actually be unbiased!!**. The fact that the mean of the residuals is 0 does not mean that they are small, it means that positive and negative residuals compensate.
