...
FEEDBACK: CODE DOES NOT RUN AND IT IS NOT CORRECT.
...
pedestrian <- pedestrian %>% index_by(date, Sensor) %>% summarise(sum(Count)) 
...
north <- pedestrian %>% filter(Sensor == "Bourke Street Mall")
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
...
```{r}
## YOUR CODE GOES HERE
southern %>% index_by(Date) %>% sumaries()
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
...
The graph is showing itsself weekly, which we say a seasonal period on 1 week from its data. If we wnat to find the value of m we will call it the lenght of the total seasional period that we could generate in the plot.

FEEDBABCK
.... m=7. Daily Data Weekly pattern.
...
YOUR ANSWER GOES HERE (Max. 30 words)
I would choose the seasonal anive model to creste forecast 
FEEDBACK: WHT
------
