...
The length of the seasonal period is one week as the changes goes in graph goes from one week to another.
...
scs_pedestrians %>%
  ...
  geom_line(aes(y = mean_val), colour = "blue")

#falta hacer dashed line
...
I will not choose Mean model because it does the mean of the variables and. It has no trend but a strong seasionality.I will choose seasonal naive model.
...
FEEDBACK: ALWAYS INCLUDE THE HISTORICA DATA
...
Both ACFs are identical because they have very similar or nearly identical autocorrelation structures.

FEEDBACK: SEE SOLUTION FOR THE TECHNICAL ANSWER.
FEEDBACK: YOU SHOULD HAVE BEEN ABLE TO PRINT ONLY THE ACFS... NOT ONLY USING RESIDUALS. PRACTICE CODING!
...
model_vals %>% as_tibble() %>%
  filter(.model == "Naive") %>%
  autoplot(model_vals, colour = "gray")
