...
pedestrian %>%
  ...
  scale_x_yearmonth(date_breaks = "5 week",
                      minor_breaks = "5 week")
```
...
The seasonal period would be around 7 days, since the season repeats every minor_break, and as I established minor_break = 1 week, That's also the seasonal period.
...
FEEDBACK: m=7 -> you would require a 7-ma, not a 2x12 Ma
...
I would personally choose a drift model, since we can appreciate that the graph has a seasonal period of 1 week, meaning that 'Count' values are quiet similar every week. Through a drift model, the forecast would be the same as the last seasonal period of the graph.

FEEDBACK: false. See solution. Seas Naive is the only one with seasonality.
...
```{r}
fc1 <- fit %>% forecast(h=7)
fc1 %>% filter(.model == "snaive") %>%
  autoplot(scs_pedestrians) +
  
scale_x_yearmonth(date_breaks = "4 week",
                      minor_breaks = "1 week")

fc <- fit %>% forecast(h=7)1
fc %>% filter(.model == "mean") %>%
  autoplot(scs_pedestrians) +
...
In this case, both ACF's are the same, we can also say that neither of them are white noise.
...
fitted_vals %>% 
  filter(.model == "mean") %>%
  autoplot(Count, colour = "black") +
  geom_line(aes(y=.resid), colour = "blue", linetype = "dashed")
```

------

Yes, the residuals of the mean model are biased.

feedback: no. The residuals are not biased, they have mean 0.
