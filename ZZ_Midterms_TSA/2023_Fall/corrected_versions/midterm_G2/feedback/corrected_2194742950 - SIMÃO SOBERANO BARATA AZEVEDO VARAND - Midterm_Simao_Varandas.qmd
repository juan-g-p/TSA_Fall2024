...
The length of the seasonal period is 1 week. Since the data is daily, the length of the seasonal period is 7. m = 7 
...
#Plot"
scs_pedestrians %>% autoplot(Count, color = "blue") +
...
This type of model forecasts based on previous information of the model and does not account for the trend. As the model has approximately no trend (seen by the red line), this model is the right fit for the forecasting. 

FEEDBACK: YOU PROVIDE NO ANSWER TO THE QUESTION.
...
fit <- scs_pedestrians %>% 
  model(
    ...
    SNaive = SNAIVE(Count ~ lag("7 days"))
    )
```
...
forecasts %>% filter(.model == "SNaive") %>% autoplot(scs_pedestrians, colour = "red")

forecasts %>% filter(.model == "mean") %>% autoplot(scs_pedestrians, colour = "blue")
...
The ACFs are the same because we are comparing the original ACF plot to the one of a mean model's.
The mean model does not caputre the seasonality of the model. Therefore, it makes sense that the ACF plots look the same.
...
fit_values %>% as_tibble() %>%
  filter(.model == "mean") %>%
  summarise(mean = mean(.innov, na.rm = TRUE))0.7

```

------

By looking at the residuals of the mean model, we can assume that they are not biased. 
This is because the value we obtain is extremely close to 0. The value is (-4.40212e-16)
