...
```{r}
## YOUR CODE GOES HERE
scs_pedestrians %>% autoplot() + scale_x_yearmonth(date_breaks="5 weeks", minor_breaks = "1 week")
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
...
We would consider a seasonal period of 1 and a half weeks, hence the pattern is repeating itself weekly.
Furthermore, the value for m would be the length of the total seasonal period that we can see in the plot.

FEEDBACK: WRONG. SEASONAL PERIOD OF 1 WEEK.
------
...
FEEDBACK: m=7. So you need a 7-ma
FEEDBACK: CODE DOES NOT RIUN

```{r}
scs_pedestriansEXT <- scs_pedestrians %>% mutate(
...
1
YOUR ANSWER GOES HERE (Max. 30 words)
I would pick the Seasonal naive model to produce forecasts of up to 1 week ahead since the seasonality is constant.
------
...
FEEDBACK: STUDY PAY ATTENTION AND PRACTICE. I do not want to, but if I have to I will fail you. And this wont cut it for the final.
