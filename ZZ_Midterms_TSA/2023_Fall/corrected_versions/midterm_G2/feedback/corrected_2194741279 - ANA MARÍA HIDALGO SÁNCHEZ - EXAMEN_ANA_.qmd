...
I would consider the seasonal period to be weekly (m = 7). I would consider this due to the fact that every week we can see an increase and a decrease of the number of pedestrians which are fixed in a period.
...
I would not choose the mean model as all the future values are equal to the average of the historical data. The drift model would better make sense if the time series had a string trend. However, as we have a time series with a strong seasonality, I would choose a seasonal naïve model that forecasts equal to the last observed value from the same season.
...
FEEDBACK: NEVER DEPICT THE FITTED VALUES UNLESS ASKED TO OR UNLESS YOU HAVE A REASON. DEPICT THE HISTORICAL DATA.

```{r}
fc_pedestrians = fit %>% forecast(h = 7)
...
```{r}
model_vals = fc_pedestrians%>% augment()
...
If there is zero mean of the residuals, there is a bias of the forecasts.

------

FEEDBACK: overall fairly good, but you need to code more and understand more.