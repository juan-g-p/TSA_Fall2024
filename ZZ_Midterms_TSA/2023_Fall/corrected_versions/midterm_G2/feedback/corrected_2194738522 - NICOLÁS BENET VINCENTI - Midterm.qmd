...
FEEDBACK: UNFORTUNATELY YOU DID NOT GET IT RIGHT. SEE SOLUTION.
...
```{r}
pedestrian_sens %>% 
  autoplot() + scale_x_yearweek(date_breaks = "5 weeks",
                      minor_breaks = "1 week")+
...
```{r}
val <- pedestrian_sens %>%
  index_by(day = ymd(Date)) %>%
  summarise(max_val = max(Count))
...
The length of the seasonal period will be 7 days (a week).The time series every 7 days is in phase hence this is the seasonal period. Therefore, the value for m will be 7.
...
scs_pedestrians <- 
  ...
  1.
  select(Date, Count, mean_val, trend_class)
...
I would pick the seasonal naïve model, since what this model does is it forecasts the last observed value from the same season, and in this case a season is a week - perfect for forecasts 1 week ahead.

FEEDBACK: WHY DO YOU SAY PERFECT? THEY ARE NEVER PERFECT. THE MEAN MODEL COULD ALSO BE AN OPTION, BUT YOU WOULD DISREGARD SEASONALITY
...
The ACF plots for the residuals of the mean model and for the actual time series look exactly the same. The residuals is just the time series values minus the mean so they have a trend and all the same features of the original time series. That is why the ACF plots are identical

FEEDBACK: EXACTLY. VERY GOOD. EXCELLENT.
...
In this case the mean is extremely close to zero, hence we can assume it as zero and deduce there is no bias in the residuals of the mean model, meaning the model is accurately capturing the trend of the data.
