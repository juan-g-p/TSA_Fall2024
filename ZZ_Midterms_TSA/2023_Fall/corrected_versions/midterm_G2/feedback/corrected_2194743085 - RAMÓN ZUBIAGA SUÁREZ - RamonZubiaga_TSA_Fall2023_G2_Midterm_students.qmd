...
FEEDBACK: STILL MORE THAN ONE DATAPOINT PER SENSOR AND DAY. NOT OK
...
```{r}
## YOUR CODE GOES HERE
max_pedestrians_day <- aggregated_data %>%
  group_by(Sensor) %>%
  top_n(n = 1, wt = Total_Counts)
...
------
I believe the seasonal period is 1 week as we could see consistent peeks every week. In this case the valu for m is 7. Being the length one week. 
...
FEEDBACK: CODE IS NOT RUNNING PROPERLY
...
------
To produce forecast up to one week ahead i would use seasonal naive model, as we asssume seasonality  repeat itself
...
------
we can observe the original time series acf count has a lot of values outside the confidence interval. I assume the other model will have its values omre into the confidence interval being white-noise. 
...
------
I couldnt get to the code but I suppose the mean model is not biased due to the fact that we can observe in the acf plot and value fall within confidence interval. 

FEEDBACK: no. Biasedness has to do with whether the mean of the residuals is 0 or not.
