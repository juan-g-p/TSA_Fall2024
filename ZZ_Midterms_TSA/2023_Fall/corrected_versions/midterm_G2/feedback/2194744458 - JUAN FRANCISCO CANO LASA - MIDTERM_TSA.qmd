...
# Generate qq_plot
plot_1 <- ggplot(mean, aes(sample = .innov))
plot_1 <- p1 + stat_qq() + stat_qq_line()

# Generate box-plot
plot_2 <- ggplot(data = mean, aes(y = .innov)) +
...
fit %>% select(dcmp_drift) %>% augment() %>% features(.innov, box_pierce, lag = 24, dof = 1)
fit %>% select(dcmp_drift) %>% augment() %>% features(.innov, ljung_box, lag = 24, dof = 1)
```
...
acf_default = stl_default %>%
  ACF(remainder) 
acf_default %>% autoplot()

acf_adjust = stl_adjust %>%
  ACF(remainder) 
acf_adjust %>% autoplot()


corr_metric_default <- sum(acf_default$acf^2)
corr_metric_adjust <- sum(acf_adjust$acf^2)

corr_metric_adjust < corr_metric_default #It's correct
