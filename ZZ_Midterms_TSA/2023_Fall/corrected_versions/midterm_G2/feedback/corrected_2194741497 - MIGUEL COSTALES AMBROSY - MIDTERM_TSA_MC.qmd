...
```{r}
## YOUR CODE GOES HERE
scs_pedestrians %>% autoplot()+ scale_x_yearmonth(
    breaks = "5 months",
    minor_breaks = "5 months"
...
The pattern in this graph shows that it is repeating itself weekly. That's why the seasonal period would be 1 week long and a half. If we had to specify the value that we would use for m , we would have called it the length of the total seasonal period that we can see in the plot. The value would be m = 7------
...
FEEDBACK: IF M=7, WHY USE A 2X12 MA???

STOP COPY PASTING CODE WITHOUT UNDERSTANDING

FEEDBACK: CODE DOES NOT RUN

```{r}
## YOUR CODE GOES HERE
scs_pedestrians1 <- scs_pedestrians %>%
  mutate(
...
The best models that I would pick to produce forecast up to 1 week ahead would be seasonal naive model because it seems that it follows a constant seasonality because we take into account first and last values.------
...
after having compared both of the plots we did, the correlation we can see
...
When anlyzing the residuals , we see that the innovation residuals also as the ACF plot, they follow the same seasonality. But when we turn into the residuals , we can see that the box plot of the residuals does not 

FEEDBACK: SEE SOLUTION

FEEDBACK: STUDY. THIS WONT CUT IT IN THE FINAL