...
pedestrian_filter <- pedestrian%>% filter(Sensor == c("Southern Cross Station","Bourke Street Mall (North)")) %>% 
  filter(Date > date("2016-07-01")) %>% filter(Date < date("2016-10-25"))
pedestrian_filter
...
```{r}
## YOUR CODE GOES HERE
pedestrian_filter %>% autoplot(Count) + 
  scale_x_datetime(date_breaks = "5 weeks",
...
```{r}
## YOUR CODE GOES HERE
southern <- pedestrian_filter %>% filter(Sensor== "Southern Cross Station") %>% head(1)
southern

northern <- pedestrian_filter %>% filter(Sensor== "Bourke Street Mall (North)") %>% head(1)
northern
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
...
I would say that the length of the period is 1 week.
...
scs_pedestrians <- 
  ...
  geom_line(aes(y = trend_class), colour = "blue") +
    geom_line(aes(y = mean_val), color = "red", linetype="dashed")
...
I would probaly use the seasonal Naive model since it generates a forecast based on the last observed value from the last season. 
------
...
fc %>% filter(.model=="snaive") %>% autoplot(scs_pedestrians, level = FALSE) + labs(title= "Snaive")

fc %>% filter(.model=="mean") %>% autoplot(scs_pedestrians, level= FALSE)  + labs(title="Mean model")
...
fitted_vals %>% filter(.model=="mean") %>% select(.innov) %>% ACF() %>% autoplot()
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
...
We can see that both of the ACF plots are completely the same. 
...
We can see that the residuals seem to be pretty homoskedastic. But we can also see that the distribution is not normal. This means that the mean model could be biased.

FEEDBACK: BIASEDNESS HAS TO DO WITH THE MEAN VALUE OF THE RESIDUALS...

FEEDBACK: STUDY MORE THIS PART OF THE THEORY. IT IS IMPORTANT
------