...
We can observe that there is a clearly repeating pattern every week that indicates weekly seasonality. With weekly data, the seasonal period would be m = 5.

daily data. repeats every week. m=7.
------
...
FEEDBACK: why are you copy pasting and not removing the irrelevant models? I did not ask you to fit all this.
...
FEEDBACK: since you copy pasted, you produced the forecasts of the wrong modes...
...
fc %>% 
  ...
  labs(title ="Forecast with the drift method")

fc %>% 
  filter(.model == "dcmp_ses") %>% 
  autoplot(scs_pedestrians) +
  labs(title ="Forecast with the SES method")
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```
