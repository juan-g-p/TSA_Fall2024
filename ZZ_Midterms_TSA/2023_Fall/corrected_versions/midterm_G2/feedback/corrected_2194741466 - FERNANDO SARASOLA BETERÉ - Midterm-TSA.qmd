...
pedestrian1 <- pedestrian  %>% index_by(Date, Sensor) %>% summarise(sum(Count, na.rm = TRUE)) 
...
```{r}
## YOUR CODE GOES HERE
southern %>% index_by(Date) %>% summarise(sum(Count))
## DO NOT CREATE ADDITIONAL CODE SNIPPETS, KEEP EVERYTHING IN A SINGLE SNIPPET.
```

FEEDBACK: CODE DOES NOT RUN PROPERLY. SOME IDEAS ARE GOOD.
...
fit %>% select(mean) %>% augment() %>% features(.innov, box_pierce, log = 24, dof = 1 )
...
FEEDBACK: STUDY AND PRACTICE AND PAY ATTENTION IN CLASS. I do not want to fail you, but I will if you keep along these lines.