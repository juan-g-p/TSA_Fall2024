...
From the ACF we can depict that the seasonal period is yearly data since it has 24 lags

feedback: FALSE: ITS YEARLY BECAUSE IT HAS POSITIVE SPIKES EVERY 12 LAGS
...
FEEDBACK: WHERE IS THE DEFAULT DECOMPOSITION? ALSO DID NOT USE APPROPRIATE NAMES

```{r, include=!params$print_sol}
# YOUR CODE GOES HERE
dcmp <- unemp %>% 
  model(
    stl_adjust = STL(unemp ~ trend(window=5) + season(window=5)),
...
FEEDBACK: FITTED ONLY ONE OF THE MODES. TRIED THE OTHER ONE

FEEDBACK: THIS DOES NOT RUN both models. see solution
...
FEEDBACK: VERY BASIC PLOTTING AND NO ANALYSIS
