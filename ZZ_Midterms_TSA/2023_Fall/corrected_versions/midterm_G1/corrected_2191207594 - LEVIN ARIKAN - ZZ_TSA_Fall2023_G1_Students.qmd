...
```{r, eval=FALSE}
unemp_data <- readr::read_csv(file.choose())
...
FEEDBACK: THIS IS WRONG, YOU ARE USING A 7-MA AND BECAUSE THE DATA IS MONTHLY, SHOULD USE A 2X12 MA. SEE SOLUTION

```{r, include=!params$print_sol}
## YOUR CODE GOES HERE
unemp_data <- unemp_data %>% mutate(
    'trend_class' = slider::slide_dbl(unemp, mean,
                                         .before = 3,
                                         .after = 3,
                                         .complete = TRUE)
  ) %>% 
  select(unemp, 'trend_class')


unemp_data %>% autoplot(unemp) + geom_line(aes(y = `trend_class`), colour = "#D55E00")
## DO NOT ADD ADDITIONAL CODE SNIPPETS.
```
...
```{r, include=!params$print_sol}
## YOUR CODE GOES HERE
unemp_data = unemp_data %>% mutate(detrended_class = unemp - trend_class)

unemp_data %>% select(detrended_class) %>% autoplot() + scale_x_yearmonth(breaks = "1 year", minor_breaks = "1 year") + theme(axis.text.x = element_text(angle = 90))
...
```{r, include=!params$print_sol}
## YOUR CODE GOES HERE
unemp_data %>% ACF(unemp) %>% autoplot()

unemp_data %>% ACF(detrended_class) %>% autoplot()
## DO NOT ADD ADDITIONAL CODE SNIPPETS.
```
...
The length of the seasonal period is 1 year (12 months) because lag 12 is the highest and the same (probably) as lag "0" and also lag 4 very similar to lag 16 --> difference 12 lags --> 12 months. Similar autocorrelation for lags with a difference of 12 months between each other.
...
```{r, include=!params$print_sol}
# YOUR CODE GOES HERE
stl_default = unemp_data %>% model(stl = STL(unemp)) %>% components()
stl_default %>% autoplot() # We can see variance of remainder component way bigger than that of trend and too similar to seasonal component

stl_adjust <- unemp_data %>% model(stl_adjust = STL(unemp ~ trend(window = 5) +
              season(window = 5))) %>% components()
...
# fc <- fit %>% select(dcmp_stl) %>% forecast(h = 12)


## FEEDBACK: IT IS NOT "UNEMP" BUT THE ORIGINAL DATASET THAT YOU SHOULD SPECIFY

fc %>% 
  filter(.model == "dcmp_stl") %>%
  autoplot(unemp, colour = "green")

fc %>% 
  filter(.model == "dcmp_drift") %>%
  autoplot(unemp, colour = "red")
# DO NOT ADD ADDITINAL CODE SNIPPETS, ALL THE CODE SHALL BE CONTAINED HERE
```
...
- Residuals are correlated due to ACF values being far outside the bounce (at least most of them).
- Visual inspection of the variance of the residuals across historical data indicates lack of homoscedasticity. Variance escpecially at the end varies strongly.
- Mean is close to the median, but data is not to normal, due to histogram skewed right and a few outliers to the right
- Mean is far away from zero
- residuals tend to show no normality and now homoscedasity
...
stl_default$season_adjust %>% augment() %>% features(.innov, box_pierce, lag = 12, dof = 1)
...
isTRUE(corr_metric_adjust < corr_metric_default)
