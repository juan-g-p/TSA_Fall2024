...
unemp %>% autoplot()+
  scale_x_yearmonth(breaks = '12 months', minor_breaks = '1 month')+
...
unemp %>%
  ...
  geom_line(aes(y = trend_class), colour = 'blue') +
  scale_x_yearmonth(breaks = '12 months', minor_breaks = '1 month')+
...
unemp <- 
  unemp %>%
  mutate(
      detrended = unemp - trend_class,
    ) %>% select(unemp, detrended)
unemp %>% autoplot(detrended)+
  scale_x_yearmonth(breaks = '12 months', minor_breaks = '1 month')+
  theme(axis.text.x = element_text(angle = 90))
```
...
```{r, include=!params$print_sol}
unemp %>% ACF(detrended, lag_max = 36) %>%autoplot()
unemp %>% ACF(unemp, lag_max = 36) %>%autoplot()

```

------

The length of the seasonal period is 12 months, and in the correlograms we can see the autocorrelation be the strongest every 12th lag. 
...
stl_adjust = 
  unemp %>%2.
  model(stl = STL(unemp ~ season(window = 5) + trend(window = 5))) %>%
  components()
...
```{r, include=!params$print_sol}
fc = fit %>% forecast(h = 12, level = c(80, 95))
fc %>% filter(.model == 'dcmp_stl') %>% autoplot(unemp) + labs(title = 'Forecast SES')
fc %>% filter(.model == 'dcmp_drift') %>% autoplot(unemp) + labs(title = 'Forecast Drift')
...
```{r, include=!params$print_sol}
aux = fit %>% augment()
mean_vals <- filter(aux, .model=="dcmp_drift")
...
FEEDBACK: THE MOST RELEVANT ASPECT IS AUTOCORRELATION


We can see based on the histogram that the residuals are not normally distributed, as it is shifted towards the left and not centered. Also looking at the qqplot we see the residuals deviate substantially from normality on the right tail. Furthermore, visual inspection of the variance shows the variance of the residuals stays nearly the same across the historical data, except for one area near to january 2020. The residuals variance can be treated as homoscedastic and not heteroskedastic. 
...
#2.
to_add_defaults = stl_defaults %>% ACF(remainder)
to_add_adjust = stl_adjust %>% ACF(remainder)
corr_metric_default = sum(to_add_defaults$acf)
corr_metric_adjust = sum(to_add_adjust$acf)
...
The adjusted decomposition is clearly better than the default, as we see the ACF's are more white noise like and the sum of the adjusted acfs is less than the defaults, meaning some of the seasonal and trend components leaked to the remainder of the default, whilst the adjusted captures mostly everything

FEEDBACK: HAD YOU SQUARED THE TERMS OR TAKEN THE ABSOLUTE VALUE... I WOULD HAVE ACCEPTED THIS.

SEE SOLUTION, THIS IS RELATED TO THE LJUNG BOX AND BOX-PIERCE STATISTICS

corr_metric_default / corr_metric_adjust SHOULD BE GREATER THAN 1
