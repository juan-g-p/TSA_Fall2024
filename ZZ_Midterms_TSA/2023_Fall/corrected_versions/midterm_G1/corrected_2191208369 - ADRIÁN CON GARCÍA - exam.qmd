...
unemp %>% autoplot() + scale_x_yearmonth(date_breaks = "1 year", minor_breaks = "1 year") + theme(axis.text.x = element_text(angle = 90))
...
FEEDBACK: THIS IS WRONG. SHOULD HAVE USED 2X12 MA. SEE SOLUTION

```{r, include=!params$print_sol}
# Centered Moving Averages 
(unemp <- unemp %>% mutate("trend_class" = slider::slide_dbl(unemp, mean, .before = 3, .after = 3, .complete = TRUE)))

# Plot 
unemp %>%
  autoplot(unemp) + geom_line(aes(y = `trend_class`), colour = "#D55E00") + labs(y = "Unemployment", title = "Unemployment in Spain")
...
```{r, include=!params$print_sol}
## YOUR CODE GOES HERE
unemp <- unemp %>% mutate(detrended_class = unemp - `trend_class`,)
unemp


(major_ticks_seq = seq(1, 21, 1))
(minor_ticks_seq = seq(1, 21, 1))
...
There's a yearly seasonality (length of 12 months). It's possible to see in the ACF of the "detrended_class", as there's a peak in 12 and presumably later at 24, and a low bottom at 6 and then 18 (12 months later).

FEEDBACK: instead of saying presumably, you could have actually depicted it specifying lag_max...
...
```{r, include=!params$print_sol}
# YOUR CODE GOES HERE
stl_default = unemp %>% model(stl = STL(unemp)) %>% components() %>% autoplot()
stl_default

stl_adjust = unemp %>%
  model(STL(unemp ~ trend(window = 7) + season(window = 7), robust = TRUE)) %>% components() %>% autoplot()
stl_adjust
...
# Generate qq_plot
qq_plot <- ggplot(model_vals, aes(sample = .innov))
qq_plot <- qq_plot + stat_qq() + stat_qq_line()
qq_plot

# Generate box-plot
boxplot <- ggplot(data = model_vals, aes(y = .innov)) +
...
The mean of the residuals is significantly close to zero, as seen by the box plot. On the other hand, the qqplot shows closeness to the line, with deviation from the legs. A constant variance can be observed, except for some outliers at the end (indicating homsocoedasiticyt).

THE MOST IMPORTANT PART IS THE AUTOCORRELATION!!

ALSO PAY ATTENTION TO THE SOLUTION, YOU SHOULD USE MORE TECHNICAL LANGUAGE.
