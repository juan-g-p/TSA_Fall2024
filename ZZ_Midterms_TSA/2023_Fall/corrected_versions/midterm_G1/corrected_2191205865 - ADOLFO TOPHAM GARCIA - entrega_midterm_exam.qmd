...
sp_arrivals1 <- sp_arrivals %>%
  ...
  select(ym, value, `7-MA`)
  

sp_arrivals1

#check
round(mean(sp_arrivals1$value[1:7]), 4) == round(sp_arrivals1$`7-MA`[[4]], 4)

trend_class <- sp_arrivals1 %>%
  model(stl = STL(value))
components(trend_class)
components(trend_class) %>% autoplot()
...
**The length of the seasonal period is of one year (12 months)**
...
```{r, include=!params$print_sol}
#obtenemos model componentes
stl_default <- sp_arrivals %>%
  model(stl = STL(value))
components(dcmp)

stl_adjust %>%
  model(
    STL(value ~ trend(window = 13) + # Manually change this value 
          # from the default 21
