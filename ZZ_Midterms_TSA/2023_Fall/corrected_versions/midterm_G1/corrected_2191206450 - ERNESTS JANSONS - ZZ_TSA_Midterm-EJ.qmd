...
The length of the seasonal period is 12 months and we see it in the ACF plot of the detrended_class.
...
stl_default <-
  unemp %>%
  model(
    stl_default = STL(unemp) #Adjust if desire other values
  ) %>%
  components()
...
stl_adjust <-
  unemp %>%
  model(
    stl_default = STL(unemp ~ trend(window=9) + season(window=5))
  ) %>%
  components()
...
fit <- unemp %>% 
  model(
    dcmp_drift = decomposition_model(
                ...
                STL(unemp ~ trend(window=9) + season(window=5)),
...
mean_vals <- filter(fitted_vals, .model=="dcmp_drift") # Choose the Mean model, or your model
...
Normality: The histogram is relatively bell-shaped, however, it has a long right-tail. This is also shown in the qq-plot where the points depart the line at the extremes. The mean is also not exactly the same as the median, hence, it is not perfectly normal.
Heteroskedasticity: The time-plot of residuals indicates that there is homoscedasticity as the points are mostly close to one another except in crisis: the GFC crisis and the pandemic. 

FEEDBACK: THE MOST IMPORTANT ASPECT IN TS IS AUTOCORRELATION!
...
#2:
#m=12, so l=24.... many observations so T/l > 5 so all is well. DOF=1
corr_metric_default <- stl_default %>% features(remainder, box_pierce, lag=24, dof=1)
corr_metric_adjust <- stl_adjust %>% features(remainder, box_pierce, lag=24, dof=1)
corr_metric_adjust[2] < corr_metric_default[2]
...
For every 1% of correlation left in the remainder of stl_default, there is only 0.6% of correlation left in the remainder of stl_adjust due to the adjusted parameters of "window".

FEEDBACK: SUPER NICELY DONE. JUST AS EXPECTED OF YOU.
------