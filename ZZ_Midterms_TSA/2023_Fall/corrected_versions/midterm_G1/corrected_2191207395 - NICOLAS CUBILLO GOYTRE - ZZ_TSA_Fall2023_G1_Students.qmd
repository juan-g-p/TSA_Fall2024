...
```
```{r}
# FEEDBACK: YOU SHOULD HAVE DEPICTED THE DETRENDED COMPONENT. SEE SOLUTION
detrended_class %>% 
  ACF(unemp) %>% 
  autoplot()
```
...
The length of the seasonal period occurs every year. we see this in the time plot where the trends occupying the seasonl periods occur evry year.

FEEDBACK: YOU WOULD HAVE SEEN IT AS WELL IN THE ACF OF THE DETRENDED COMPONENT!
...
```{r}
library(patchwork)
# FEEDBACK: LIBRARIES ARE TO BE LOADED AT THE TOP!
```
...
```{r}
nv_vals <- filter(model_vals, .model=="dcmp_drift")
...
```{r}
nv_vals <- filter(model_vals, .model=="dcmp_stl")
...
THE GRAPHS ARE OK, THE INTERPRETATION IS VERY BAD.

We casn observe there are some outliers when looking at the graphs. This is seen as plots dont reflect white noise meaning that residuals are from of from the mean. This thereofre means that plots from residuals differ a lot from the mean menaing plots are homodastic. We can see that the first three lags and last 3 lag  autocorrelation coefficients are not within the boundaries of negligible autocorrelation. However the rest 3 lags are. THis suggests as statded before that this are a homodastic plots.
...
max_err_seasonal = max(classical_dec$seasonal - manual_decomposition$seasonal_c.x, na.rm = TRUE)
max_err_trend = max(corr_metric_adjust)
max_err_rand = max(_dec$remainder - manual_decomposition$remainder, na.rm = TRUE)
