...
autoplot(unemp) + 
  ...
  labs(title = "Unemployment Rate in Spain", x = "Year", y = "Unemployment Rate") +
  theme_minimal()
```
...
The length of the seasonal period can be seen in the second ACF, which shows significant peaks at regular intervals. The interval between these peaks indicates the seasonal period.
...
corr_metric_default <- sum(acf_default$ACF[-1]^2)
corr_metric_adjust <- sum(acf_adjust$ACF[-1]^2)

result <- corr_metric_default / corr_metric_adjust
```
