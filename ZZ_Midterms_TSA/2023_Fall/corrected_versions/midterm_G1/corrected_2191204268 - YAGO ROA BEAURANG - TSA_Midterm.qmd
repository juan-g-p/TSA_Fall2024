...
unemp %>% 
  autoplot + 
  scale_x_yearmonth(date_breaks = "4 years", minor_breaks = "1 year")
...
FEEDBACK: YOU WERE SUPPOSSED TO USE A MOVING AVERAGE, NOT DO THIS. JUST LIKE WE DID FOR THE GROUP ASSIGNMENT

ALSO THIS CODE IS COMPLETELY DIFFERENT FROM ANYTHING I HAVE SHOWN IN CLASS AND YOU ARE USING LOESS INTEAD OF A MOVING AVERAGE.

OBVIOUSLY YOU HAVE COPIED THIS FROM CHAT-GPT OR SOME INTERNET SOURCE WHEN THAT WAS FORBIDEN SO I SHOULD INSTANTLY FAIL YOU.
...
ggplot(unemp, aes(x = date, y = unemp)) +
  ...
  geom_line(aes(y = unemp), color = "red", size = 1) +  
  labs(x = "Year-Quarter", y = "Value") +
  scale_x_yearquarter(date_breaks = "4 years", minor_breaks = "1 year") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90))


unemp <- unemp %>%
  mutate(trend_class = fitted(loess(unemp ~ as.numeric(date))))

ggplot(unemp, aes(x = date, y = trend_class)) +
  geom_line(color = "blue") + 
  geom_line(aes(y = trend_class), color = "red", size = 1) +  # Trend in red
  labs(x = "Year-Quarter", y = "Value") +
  scale_x_yearquarter(date_breaks = "4 years", minor_breaks = "1 year") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90))
...
FEEDBACK: I CANNOT COUNT THE AMMOUNT OF TIMES WE HAD DONE THIS ADJUSTING EXERCISE IN CLASS

YOU ONLY FITTED OK ONE DECOMPOSITION
...
stl_adjust <- 
  unemp %>%
  model(stl_adjusted = STL(unemp, robust = TRUE)) %>%
  components()
...
dcmp_drift <- stl_decomp %>%
  model(
    drift = TSLM(.model ~ trend()),
    seasonal_naive = NAIVE(season)
  )


dcmp_drift <- dcmp_drift$model$drift + dcmp_drift$model$seasonal_naive
```
...
FEEDBACK: CODE DOES NOT RUN. CODE APPLIED TO THE WRONG MODELS.

CODE NOT USED IN CLASS AT ALL, SYNTAX COMPLETELY DIFFERENT AND COPIED FROM THE INTERNET

I SHOULD FAIL YOU IMMEDIATLY WITHOUT FURTHER CONSIDERATIONS
...
corr_metric_default <- Acf(stl_default[, "remainder"], lag.max = 24, main = "ACF of Remainder stl_default")
corr_metric_default

corr_metric_adjust <- Acf(stl_adjust[, "remainder"], lag.max = 24, main = "ACF of Remainder stl_adjust")
corr_metric_adjust

acf_default <- Acf(stl_default[, "remainder"], lag.max = 1, plot = FALSE)$acf[2]
acf_adjust <- Acf(stl_adjust[, "remainder"], lag.max = 1, plot = FALSE)$acf[2]
...
FEEDBACK: STUDY, PRACITCE AND PAY ATTENTION IN CLASS

THIS WONT CUT IT FOR THE FINAL

DO NOT PUT ME IN A SITUATION WHERE I HAVE TO FAIL YOU, BECAUSE I WILL IF I HAVE TO.