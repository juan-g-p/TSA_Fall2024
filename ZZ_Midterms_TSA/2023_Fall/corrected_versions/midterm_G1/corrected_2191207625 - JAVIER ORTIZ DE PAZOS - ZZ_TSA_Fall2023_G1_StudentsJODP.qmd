...
manual_decomposition %>% autoplot(detrended_class) + scale_x_yearmonth(breaks = "1 year",
                                                                       minor_breaks = "1 year") +
  theme(axis.text.x = element_text(angle = 90))
...
manual_decomposition %>%
  ACF(detrended_class, lag_max = 48) %>%
  autoplot() +
  labs(title="Unemployed people in Spain")
...
In the original time series there is a clear trend pattern high correlation and decently slow decreasing correlation as seen between the lags

However the seasonnal period can be seen in the detrended time series because in the first acf plot the trend is so strong that seasonality cant be spotter but in the detrended one we can see a clear 12 lag seasonal period (1year)
...
```{r, include=!params$print_sol}
fit %>% 
  select(dcmp_ses) %>% # Select only dcmp_drift
  gg_tsresiduals()
```

------
Normality-> The histogram suggest that the distribution is not fully normal, particularly due to the long right tail where the couple outliers to the right of it. 

heteroscedasticity -> There is certain homoscedasticity on the residuals as seen from the scatter plot above of innovation residuals

FEEDBACK: THE MOST IMPORTANT ASPECT INTIME SERES IS AUTOCORRELATION. OH MY GOD, NOBODY IS DOING THIS... THERE ARE 4 CRITERIA YOU NEED TO CHECK

ALSO VERY BASIC PLOTTING. QQPLOT AND BOX-PLOT FOR NORMALITY WOULD HAVE BEEN APPRECIATED
------
...
1. directly from plots we can see that the remainded of the default stl dec is not white noise while the one of the adjusted is. This is good as it tells us that the adjustment was done correctly
------