...
FEEDBACK: YOU WERE SUPPOSSEDT TO USE A 2X12 MA AS IN THE GROUP ASSIGNMENT
...
FEEDBACK: THIS DOES NOTHING OF WHAT I ASK YOU TO DO.
...
The period is every year as expected, because data is storage every year.

FEEDBACK: ANSWER MAKES NO SENSE. SEE SOLUTION.
...
FEEDBACK: THESE ARE NOT THE MODELS I ASKED YOU TO FIT

```{r, include=!params$print_sol}
1.
dcmp_drift <- unemp %>% model(Drift = RW(unemp ~ drift()))
dcmp_drift

2.
dcmp_stl <- unemp %>%
  model(
    unemp = ETS(unemp ~ error("A") + trend("N") + season("N"))
  )
...
FEEDBACK: VERY BASIC CODING AND NO ANALYSIS
...
2. Compute a single number that measures the amount of autocorrelation left in the remainder of each decomposition.
  ...
  * Compute `corr_metric_default / corr_metric_adjust`. Give a short interpretation of the result
  ...
  FEEDBACK. IN BOTH CASES YOU ARE NOT PLOTTING THE ACF OF THE REMAINDER. SEE SOLUTION
...
OBCIOUSLY YOU NEED TO STUDY A LOT, PRACTICE A LOT AS WELL AS TO PAY ATTENTION IN CLASS.
