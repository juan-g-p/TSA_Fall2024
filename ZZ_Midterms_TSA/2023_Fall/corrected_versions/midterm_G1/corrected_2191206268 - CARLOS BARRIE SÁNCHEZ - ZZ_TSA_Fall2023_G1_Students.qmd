...
FEEDBACK: THIS IS WRONG. SEE SOLUTION YOU ARE MISSING THE SECOND AVERAGING... 2X12...

```{r, include=!params$print_sol}
# 1 and 2:
mov_average_unemp <- unemp %>%
  mutate(
...
# Store the results under "detreded_class":
classical_dcmp <- 
  classical_dcmp %>% 
  mutate(
...
classical_dcmp %>% 
  autoplot(detrended) + 
  scale_x_yearmonth(breaks = "1 years", minor_breaks = "1 year") + 
  theme(axis.text.x = element_text(angle = 90))
...
------
### The first ACF plot shows a strong and positive correlation that decays slowly as lags increase. Regarding the second ACF plot we can clearly apreciate a seasonality pattern with spikes at lag 12 which actually matches the time series.
------
