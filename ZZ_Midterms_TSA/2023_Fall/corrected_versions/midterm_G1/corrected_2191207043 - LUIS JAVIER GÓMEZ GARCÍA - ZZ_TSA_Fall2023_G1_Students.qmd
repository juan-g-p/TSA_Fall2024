...
FEEDBACK: PAY ATTENTION, THE WAY YOU SPECIFY THIS IS SUCH THAT YOU HAVE TWO LINES PER END OF YEAR...
...
unemp_ma <- 
  ...
  # Compute new column containing the detrended componen
      
  mutate(
    detrended = unemp - unemp_ma$trend_class,
    detrended = unemp - trend_class # FEEDBACK: This is enough
  )
...
unemp_ma %>% ACF(detrended) %>% autoplot()
## DO NOT ADD ADDITIONAL CODE SNIPPETS.
```

------------------------------------------------------------------------

**The length of the seasonal period is of around 5 to 6 months, as can be seen in the detrended ACF plot.**

WRONG: THE LENGTH OF THE SEASONAL PERIOD IS 1 YEAR. STRONG POSITIVE CORRELATION EVERY M LAGS. BESIDES, THE SEASONAL PATTERN IS TO BE SEEN IN THE TIME-PLOT. THE NEGATIVE SPIKE EVERY SIX MONTHS INDICATES HALF THE SEASONAL PERIOS (LAGGED SERIES IN PHASE OPPOSITION). THIS WAS EXPLAINED FOR REALLY LONG IN CLASS... AND REPEATED MANY TIMES.
...
FEEDBACK: WHERE IS THE FIGURE I REQUESTED??

```{r, include=!params$print_sol}
# YOUR CODE GOES HERE
fc <- fit %>% 
  forecast(h = 12) %>%
  filter(.model == 'dcmp_stl' | .model == 'dcmp_drift')
fc
# DO NOT ADD ADDITINAL CODE SNIPPETS, ALL THE CODE SHALL BE CONTAINED HERE
```
...
The residuals are clearly correlated, as it can be seen in the ACF plot (most lags are out of the boundaries for white noise). Moreover, the residuals seem to be somewhat homoscedastic as the variance remains mostly uniform for the historical data. However, there is a strong increase towards the most recent observations with a strong presence of outliers. The data doesn't seem to follow a normal distribution, as it strays from the normality trend in the QQ plot and the mean is different from 0 which indicates forecasts using this model will be biased.
...
## YOUR CODE GOES HERE
remdefault <- stl_default %>% 
  ACF(remainder) 

remdefault %>% autoplot()

remadjusted <- stl_adjusted %>%
  ACF(remainder) 

remadjusted %>% autoplot()


remdefault %>% select(acf) 
