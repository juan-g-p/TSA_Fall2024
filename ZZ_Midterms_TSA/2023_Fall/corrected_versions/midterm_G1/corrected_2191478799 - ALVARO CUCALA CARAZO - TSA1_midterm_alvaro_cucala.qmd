...
unemp %>% autoplot() + scale_x_yearmonth(date_breaks = "3 year",
                                         minor_breaks = "1 year")
...
unemp_ma %>%
  ...
  scale_x_yearmonth(date_breaks = "3 year", minor_breaks = "1 year") + 
...
classical_dcmp %>% 
  autoplot(detrended) +  scale_x_yearmonth(date_breaks = "3 year", minor_breaks = "1 year")
...
The length of the seasonal period is of 12 months (1 year. This can clearly be seen as the peaks of the detrended time series ACF plot are every 6 months, meaning a cycle every 12 months.

FEEDBACK: WRONG INTERPRETATION. THIS CAN BE SEEN IN THE POSITIVE CORRELATIONS EVERY 12 LAGS
...
fc_dr <- dcmp_drift %>% forecast(h = 12)

fc_stl <-  dcmp_stl %>% forecast(h=12)
...
------------------------------------------------------------------------
FEEDBACK: THE MOST IMPORTANT FEATURE IN A TS MODEL IS WHETHER RESIDUALS ARE CORRELATED OR NOT...

ALSO SOME ADDITIONAL GRAPHS TO IMPROVE THE ANALYSIS ARE MISSING



The histogram for both models suggest that the distribution is not fully normal, particularly due to the long right tail where the couple outliers accumulate. 


The residuals for both models seem fairly homoscedastic, the variance of the residuals remains more uniform across the historical data, with exception to the crisis in 2008 and 2020 (covid), that could be treated as outliers.
------------------------------------------------------------------------
...
# I believe it has to do with finding the standard deviations and computing a prediction interval. 
# We expect 95% of the spikes in the ACF to lie within , where is the length of the time series. Checking this is similar to performing separate statistical tests on each autocorrelation coefficients. Therefore, we may have false positives.
# but i ran out of time
## DO NOT ADD ADDITIONAL CODE SNIPPETS
```

------------------------------------------------------------------------
The residuals are highly correlated, as the correlogram clearly shows (ACF values outside the dashed bounds in both plots).
