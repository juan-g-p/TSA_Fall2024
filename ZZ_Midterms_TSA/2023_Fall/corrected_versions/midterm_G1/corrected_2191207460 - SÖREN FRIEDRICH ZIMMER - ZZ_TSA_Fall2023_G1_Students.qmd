---
title: "ZZ_TSA_G1_2023Fall_Midterm Sören Zimmer"
editor: source
params:
  print_sol: false
toc: true
...
FEEDBACK: YOU ARE TAKING A AN MA OF ELEVEN POINTS. THIS IS NOT WHAT CLASSICAL DECOMPO DOES (5 + 5 + 1 = 11)
...
The seasonal period is 12 months, a yearly seasonality. We can see this clearly as there is a peak (or starting at lag 6 a low) every 12 months. This can only cleary be seen in the detrended data, as the trend component earlier was too influential on the data.
...
FEEDBACK: WHY ARE YOU FITTING ADDITIONAL MODES? YOU GUYS NEED TO STOP SIMPLY COPY PASTING WITHOUT UNDERSTANDING
...
FEEDBACK: THE MOST IMPORTANT ASPECT OF THE RESIDUALS IN A TS MODEL IS AUTOCORRELATION

Normality: The residuals based on the histogram look relatively normal with few outliers. To confirm I conducted a Box Pierce test which came back with extremly low p value, so the normality can be seen as confirmed.

Heteroskedasticity: Because of the crises (banking 2008, Covid 2019) we do have some unexpected spikes in the data. Still, the residuals mostly have a stable variance, so the assumption is met. Also, the mean of the residuals is around zero and close enough to the median.
------
...
The acf1 is the autocorrelation coefficient of the series, acf10 the sum of squares of the first ten autocorrelation coefficients. 
Both are significantly smaller in the adjusted series meaning that the adjusted series is much more efficient in "taking out" the remainder, meaniung the remainder is really ONLY a remainder and has no explanable (correlation) but rather just random components in it.

Very good. You just forgot to do the ratio and difference I requested.
