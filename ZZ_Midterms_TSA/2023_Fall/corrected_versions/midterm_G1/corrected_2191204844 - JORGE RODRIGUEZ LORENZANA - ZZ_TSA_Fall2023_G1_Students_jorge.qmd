...
Unemp %>% autoplot() + scale_x_yearmonth(breaks = "1 year", minor_breaks = "1 year") + theme(axis.text.x = element_text(angle = 90))
```
...
FEEDBACK: YOU WERE SUPOSSED TO USE A MOVING AVERAGE, JUST AS WE HAD DONE FOR THE GROUP ASSIGNMENT.

```{r, include=!params$print_sol}
Unemp %>% model(dcmp = classical_decomposition(unemp, type = "additive")) %>% rename(trend = 'trend_class') %>% components()
  

Unemp %>% autoplot(unemp) + geom_line(aes(y = 'trend'), color = "red")
```
...
FEEDBACK: OBVIOUSLY YOU HAVE NOT PAID ATTENTION AT ALL IN CLASS OR STUDIED ANYTHING

```{r, include=!params$print_sol}
Unemp %>% select(-trend_class)
```
...
```{r, include=!params$print_sol}
stl_default <- Unemp %>% model(stl = STL(unemp)) %>% components() 

stl_default %>% autoplot

stl_adjust <- Unemp %>% mutate(inv_unemp = -1/unemp) %>% model(stl =STL(inv_unemp)) %>% components()
...
FEEDBACK: EVEN THIS IS NOT RIGHT, YOU INCLUDE AUGMENT AT THE END. CLEARLY YOU DO NOT KNOW WHAT YOU ARE DOING
...
fit %>% select(.model == 'dcmp_ses') %>% autoplot(unemp)

fc <- fit %>% forecast(h = 12)

fc %>% autolpot(unemp, level = FALSE)
  
fit %>% filter(.model == "dcmp_drift") %>% autoplot(unemp) 
...
```{r, include=!params$print_sol}
mean_vals <- filter(fit, .model=="dcmp_drift")
...
By looking at the boxplot (closeness of mean and median), Q-Q plot (crosses over the line), and the residual plot/histogram (a bit symmetric and slightly bell shaped), we can see that the residuals are fairly normal with a mean of 0 but obviously it could be better. As for homoscedasticity, it is not very homscedastic as the acf plot has a fairly strong trend component. 
...
corr_metric_default <- stl_default %>% features(.innov, box_pierce, lag = 24, dof = 1)

stl_adjust %>% ACF(remainder) %>% autoplot()

corr_metric_adjust <- stl_default %>% features(.innov, box_pierce, lag = 24, dof = 1)
```

------

The autocorrelations of the stl_default decomposition are larger for the most part that the stl_adjust. The stl_adjust autocorrelation resembles white noise as the autocorrelations are closer to 0, meaning that this decompostition is preferred.

FEEDBACK: NO METRICS COMPUTED

------



FEEDBACK: CLEARLY YOU COPIED LAST PART SINCE IT IS MUCH BETTER THAN THE REST OF THE EXAM AND IT COVERS MORE ADVANCED CONCEPTS

FEEDBACK: THIS WONT CUT IT FOR THE FINAL.

FEEDBACK: STUDY. AND PRACTICE. AND PAY ATTENTION AND DO NOT TALK IN CLASS.