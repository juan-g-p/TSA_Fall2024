...
FEEDBACK: this is wrong. With this approach you are considering a window of 24 points. This is not a 2x12MA, which is what you needed
...
unemp <- unemp %>%
  mutate(trend_class = slider::slide_dbl(unemp, mean, .before = 11, .after = 12))
...
ggplot(unemp, aes(x = date)) +
  geom_line(aes(y = detrended_class)) 
...
FEEDBACK: CODE DOES NOT RUN. SEE SOLUTION
...
FEEDBACK: WITH THIS CODE YOU ARE NOT PROPERLY ADAPTING THE WINDOW. ONLY STL DEFAULY IS CORRECT. PERIODIC MAKES THE WINDOW ACTUALLY BIGGER INSTEAD OF SMALLER. SEE SOLUTION
...
stl_default <- unemp %>%
  model(stl_default_model = STL(unemp)) %>%
  components()
stl_adjust <- unemp %>%
  model(stl_adjust_model = STL(unemp, s.window = "periodic")) %>%
  components()
autoplot(stl_default, type = "components")
autoplot(stl_adjust, type = "components")
...
FEEDBACK: CODE IS WRONG AND CHOICE OF WINDOW IS WRONG
...
dcmp_drift <- unemp %>%
  model(dcmp_drift = decomposition_model(
    STL(unemp, s.window = "periodic"),
    season_adjust = drift(),
    season = seasonal_naive()))

dcmp_stl <- unemp %>%
  model(dcmp_stl = decomposition_model(
    STL(unemp, s.window = "periodic"),
    season_adjust = ETS("ANN"),
    season = seasonal_naive()))

 mutate(dcmp_drift_forecast = forecast(dcmp_drift, h = "2 years"))
 mutate(dcmp_stl_forecast = forecast(dcmp_stl, h = "2 years"))
...
FEEDBACK: JUST BASIC CODE THAT DOES NOT EVEN RUN
...
FEEDBACK: YOU NEED TO STUDY. A LOT. NOW.    0
