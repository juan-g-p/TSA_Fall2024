...
```{r, include=!params$print_sol}
#1. Computing moving average (window = 12 because 12 is the seasonal period length)
unemp <-
  unemp %>%
  mutate(
...
unemp %>%
  ...
  geom_line(aes(y = trend_class), color = "#D55E00") +
  scale_x_yearmonth(breaks = "1 year") +
  theme(axis.text.x = element_text(angle = 90))
...
```{r, include=!params$print_sol}
# 1. ACF plot of original time series:
unemp %>%
  ACF(unemp, lag_max=36) %>%
  autoplot()

#2. ACF plot of detrended TS:
unemp %>%
...
In the ACF plot of the original TS we can't visualize the seasonal period since the trend is much stronger so its effect dominates. In the detrended TS we can appreciate the seasonality happening every year (seasonal period = 12 months), which we can see with high positive correlations in lags 12, 24, 36; and strong negative correlations at lags 6, 18, 30. 

FEEDBACK: nicely done. Great :))
...
#2. STL with adjusted parameters:
stl_adjust <-
  unemp %>%
  model(
...
fit <-
  ...
  model(
    dcmp_drift = decomposition_model(
      ...
      #Model for seasonally adjusted comp:
      RW(season_adjust ~ drift()),
      
      #Model for seasonal comp:
      SNAIVE(season_year)
    ),
...
#1. Plot for model dcmp_drift
fc %>%
  filter(.model == "dcmp_drift") %>%
  autoplot(unemp)

#2. Plot for model dcmp_stl:
fc %>%
...
The histogram of the residuals is relatively bell-shaped, but it has a strong right tail due to some outliers, so the distribution is not fully normal. This is also seen in the QQplot since there is a strong deviation from the diagonal at the extremes, and in the box-plot which is asymmetric; therefore the condition of normality is not fulfilled. 
Looking at the time plot of residuals we can see that their variance is pretty stable until around 2007, then it is high until 2010, then they become more stable again, and around 2018 the variance is very high again. Therefore the data is not homoskedastic (we have 2 clear clusters of higher variance)

FEEDBACK: OK... BUT MOST IMPORTANT FEATURE IN A TS SERIES MODEL IS AUTOCORRELATION!!
...
#Ljung-Box test to measure the amount of correlation:
#seasonal data. l = 2*m = 24
test_default <- stl_default %>% features(remainder, ljung_box, lag = 24, dof = 1)
corr_metric_default <- test_default[2]
test_adjust <- stl_adjust %>% features(remainder, ljung_box, lag = 24, dof = 1)
corr_metric_adjust <- test_adjust[2]
...
The result of corr_metric_default / corr_metric_adjust is around 3,5. This means that the statistic for the test of the default model is 3.5 times bigger than the statistic of the test for the adjusted model. The statistic will be greater as the autocorrelation of the different lags increases. Therefore, this proves that the autocorrelation of the remainders of the adjusted model is much lower than that of the default model (which is better).

FEEDBACK: 10 OUT OF 10. VERY GOOD. CONGRATS. GO CELEBRATE :).
