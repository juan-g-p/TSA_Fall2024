...
FEEDBACK: OK YOU DETRENDED BUT FAILED TO DEPICT IT.

FEEDBACK: PLEASE UNDERSTAND EVERYTHING YOU DID WRONG HERE. AND PRACTICE YOUR CODE
...
classical_dcmp <- 
  ...
  mutate(
    detrended_class = unemp - trend #SUBTRACT
  )
...
ACF_detrended<- 
  unemp%>%
  mutate(detrended = unemp - `2x4-MA`,
    
  )
ACF_detrended%>%autoplot(detrended)
...
FEEDBACK: YOU FIT TOO MANY MODELS. STOP DOING SOLELY COPY PASTING. THIS IS NOT LEARNING.
...
FEEDBACK: OBVIOUSLY YOU HAVE NO CLUE WHAT YOU ARE DOING. 

HOW COME YOU FIT MODELS BEFORE AND NOW REFIT AGAIN TO PERFORM FORECASTS? PAY ATTENTION IN CLASS, STUDY AND PRACTICE. WHAT WE ARE DOING IS FAILY EASY AND METHODICAL... 
...
fit <- unemp_fil %>%
  model(
    ses_unemp = ETS(unemp ~ error("A") + trend("N") + season("N"))
  )
...
depict_stl_default<- stl_default %>% 
  ACF(remainder) %>% 
  autoplot()

depict_stl_adjust<- stl_adjust %>% 
  ACF(remainder) %>% 
  autoplot()
...
FEEDBACK: THIS WONT CUT IT FOR THE FINAL. THE FACT THAT YOU HAVE PASSED DOES NOT MEAN YOU CAN RELAX. I HAVE BEEN VERY FORGIVING.