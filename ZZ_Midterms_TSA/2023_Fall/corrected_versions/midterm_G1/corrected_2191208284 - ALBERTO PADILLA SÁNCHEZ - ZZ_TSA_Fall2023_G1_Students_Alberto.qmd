...
# YOUR CODE GOES HERE
seq_months <- yearmonth("2001 December") + seq(0, 252 , by = 12)
seq_months

seq_months1 <- yearmonth("2001 December") + seq(0, 252 , by = 12)
seq_months1

unemp %>% autoplot() + scale_x_yearmonth(breaks = seq_months, minor_breaks = seq_months1)
...
trend_class %>% autoplot(detrended_class) + scale_x_yearmonth(breaks = seq_months, minor_breaks = seq_months1)
...
trend_class %>% ACF() %>% autoplot()

trend_class %>% ACF(detrended_class) %>% autoplot()
## DO NOT ADD ADDITIONAL CODE SNIPPETS.
```
...
FEEDBACK: WHERE IS THE ANALYSIS?
...
stl_adjust = unemp%>%
  model(
    STL(unemp ~ trend(window = 5) + # Manually change this value 
                                            # from the default 21
                   season(window = 5), # Replace with "periodic" 
...
The graph seems fairly homostatic (feedback: homoskedastic...), except for the 2021 peak, and the 2009 peak. Taking that, the variance doesn't change much along the graph

Also, the qqplot, the boxplot and the histogram doesnt show normality, as box plot isnt uniform and the qqplot shows in both lags there is no normality

FEEDBACK: THE MOST RELEVANT ASPECT IN A TIME SERIES MODEL IS AUTOCORRELATION!!!
...
corr_treic_default =  stl_default %>% ACF()
corr_metric_adjust =  stl_adjust %>% ACF()
