...
manual_decomposition <- unemp %>%
  mutate(
    ...
    `2xtrend_class` = slider::slide_dbl(`trend_class`, mean,
...
manual_decomposition %>%
  ...
  geom_line(aes(y = `2xtrend_class`), colour = "#D55E00") +
    scale_x_yearmonth(date_breaks = "1 year",
                      minor_breaks = "1 year") +
...
The length of the seasonal period is 12 months as we can see in the lags every 12 months (18, 30, 42...). We can not see the seasonal component in the first ACF because the tend is so strong, but in the second AFC of the detrended one, we see it perfectly.
...
```{r, include=!params$print_sol}
fit %>% 
  select(dcmp_ses) %>% # Select only dcmp_ses
  gg_tsresiduals()
```

------------------------------------------------------------------------

FEEDBACK: the most relevant feature of the residuals in a TS model is autocorrelation. Not mentioned in your text analysis

FEEDBACK: ALSO SOME GRAPHS MISSING TO COMPLETE THE ANALYSIS: qqplot, box-plot... 

For the dcmp_drift model, we could fairly assume normality as the histogram shows that most of the values are in 0 value and has a bell distribution, eventhough we have some outliers to the right of the histogram. Related to homoscedasticity we can appreciate a fairly homoscedasticity until 2008 (due to the crisis) and 2020 (where covid appears). The dcmp_ses model shows almost identiccal outputs.
...
#2
acf_corr <- stl_default %>%
  ACF(remainder)

corr_metric_default <- mean(acf_corr$acf)

acf_corr2 <- stl_adjust %>%
  ACF(remainder)

corr_metric_adjust <- mean(acf_corr2$acf)
...
By seeing these results we interpret that the adjusted metric has a lower correlation than the default metric.

FEEDBACK: the metrics you are picking up do not work... corr_metric_default / corr_metric_adjust should be bigger than 1 because the adjsuted decomposition has a remainder with smaller autocorrelation.

Nice try though. the main problem is that you did not square them...

But mainly I was expecting you to use the box-pierce or ljung-box statistics, both of which can be considered a metric of the amoutn of autocorrelation in the series.
