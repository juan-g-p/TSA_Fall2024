...
FEEDBACK: THIS IS NOT A 2X12 MA, BUT RATHER A 2X4 MA. CONCEPTUALLY WRONG
...
manual_decomposition %>% mutate( trend_class = `2x12-MA`)
...
manual_decomposition%>%autoplot(detrended_class) + scale_x_yearmonth(date_breaks = "1 year") +
  theme(axis.text.x = element_text(angle = 90))
...
the length of the seasonal period is 9 lags and can be seen in the detrended ACF

```{r, include=!params$print_sol}
## YOUR CODE GOES HERE
ACF(unemp)%>%autoplot()
manual_decomposition%>% ACF(detrended_class)%>%autoplot()
## DO NOT ADD ADDITIONAL CODE SNIPPETS.
```
...
FEEDBACK: WHY DID YOU FIT THIS MODEL???? I EVEN FITTED THESE MODELS IN THE MIDTERM MOCKUP...

YOU CLEARLY NEED TO STUDY. THIS WONT CUT IT FOR THE FINAL.

```{r, include=!params$print_sol}
# YOUR CODE GOES HERE
dcmp <- unemp %>% model(Drift = RW(unemp ~ drift()))
dcmp_drift <- dcmp %>% augment()
dcmp_drift %>% 
  filter(.model == "Drift") %>%
...
FEEDBACK: VERY BASIC PLOTTING AND NO ASSESSMENT
...
FEEDBACK: I DO NOT THINK I HAVE TO SAY ANYTHING. YOU NEED TO STUDY AND PRACTICE TO PASS THIS SUBJECT.

I SUGGEST YOU START WITH THE BASICS OF R AND GO THROUGH ALL THE NOTEBOOKS AND EXERCISES.