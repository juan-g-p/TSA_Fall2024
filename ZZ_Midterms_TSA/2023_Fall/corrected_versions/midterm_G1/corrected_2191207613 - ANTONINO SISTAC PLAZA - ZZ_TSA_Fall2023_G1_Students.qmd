...
seq_months <- yearmonth("1999-12-01") + seq(12, 12*23, by = 12)
...
FEEDBACK: THIS IS WRONG. YOU NEEDED A 2X12 MA AS CLASSICAL DECOMPOSITION WOULD USE.

FEEDBACK: GRAPH DOES NOT RUN. SEE CODE AND UNDERSTAND WHAT EXACTLY IS WRONG WITH YOUR CODE
...
unemp = unemp %>%
  mutate(
    detrend_class = unemp - trend_class
  )
unemp %>%
  autoplot(detrend_class) +
  scale_x_yearmonth(
    breaks = seq_months,
    minor_breaks = seq_months
  ) +
...
The length of the seasonal period is 12 months, and it can be perfectly observed in the ACF plot of the detrended time series
...
FEEDBACK: THE MOST IMPORTANT FEATURE IN THE RESIDUALS OF A TS MODEL IS THE AUTOCORRELATION!!!!!!!!!!!

The variance of the residuals remains pretty uniform across the historical data,so we can conclude that the  residuals are pretty homoscedastic. The other values can be considered outliers. Visually the residuals seem to be normally distributed. The histogram seems symmetric and relatively bell-shaped. The values adjust well in the qq-plot with the exception of the tails.

The box-plox confirms the symmetry of the distribution and the closeness of mean and median.
...
cor(stl_default$remainder,stl_default$trend+stl_default$season_year)?
...
FEEDBACK: NO SOLUTION AND PAINTED THE SAME ACF TWICE
