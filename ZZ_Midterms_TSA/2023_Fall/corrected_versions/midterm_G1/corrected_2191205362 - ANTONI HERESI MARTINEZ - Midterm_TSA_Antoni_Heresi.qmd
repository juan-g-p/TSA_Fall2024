...
FEEDBACK: DID NT COMPUTE MOVING AVERAGE AND PAINTED WRONG GRAPH.
...
unemp %>%
  autoplot(unemp) +
  autolayer(trend_class, series = "Trend", color = "red") +
...
FEEDBACK: CODE DOES NOT RUN. YOU DO NOT HAVE TREND CLASS. ALSO NO GRAPH
...
The length of the seasonal period is 12 months. We can see it in the ACF plot of the detrended time series, as we see that the first peak is at lag 12.
...
stl_adjust <- unemp %>%
  model(stl = STL(unemp, robust = TRUE, s.window = "")) %>%
  components()
...
FEEDBACK: CODE DOES NOT RUN. AND THIS CODE IS OVERLY COMPLEX AND WAS CLEARLY GENERATED USING "SOME TOOL" YOU SHOULD NOT HAVE USED...
...
fc %>%
  ...
  autolayer(fc$mean + 2 * fc$level, series = "95% Confidence Interval", color = "red") +
  autolayer(fc$mean - 2 * fc$level, series = "", color = "blue") +
  scale_x_yearmonth(date_breaks = "1 year", date_labels = "%Y") +
  theme(axis.text.x = element_text(angle = 90))
```
...
fit %>%
  ...
  gg_tsresiduals() +
  ggtitle("Residuals of the dcmp_drift model")

fit %>%
  filter(.model == "dcmp_drift") %>%
  gg_tsresiduals() %>%
  ACF() +
  ggtitle("ACF of the residuals of the dcmp_drift model")
...
fit %>%
  ...
  ACF() %>%
  Box.test() +
  ggtitle("Box.test of the residuals of the dcmp_drift model")

```

------

FEEDBACK: VERY BASIC ANALYISIS. QQPLOT AND BOX-PLOT TO BETTER ASSESS NORMALITY MISSING. AT LEAST YOU MENTIONED AUTOCORRELATION


The residuals of the model are not normally distributed. The residuals are also heteroskedastic, as we can see in the ACF plot. The residuals are not independent, as we can see in the ACF plot. The residuals are not white noise, as we can see in the ACF plot.

------

FEEDBACK: YOU NEED TO STUDY MUCH MORE AND NOT RELY ON "SOME TOOLS" YOU SHOULD NOT USE. THIS WONT CUT IT FOR THE FINAL.
