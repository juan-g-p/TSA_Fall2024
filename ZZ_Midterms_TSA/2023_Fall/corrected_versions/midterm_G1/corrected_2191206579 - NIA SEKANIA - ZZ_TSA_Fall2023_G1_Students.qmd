...
FEEDBACK: YOU NEEDED A 2X-12 MA, NOT A 7-MA
...
trend_class %>% 
  ACF(unemp) %>% 
  autoplot()
# ACF for detrended time series0
detrended_class %>% 
  ACF(unemp) %>% 
  autoplot()
```
...
ACF exhibits a clear trend. 

 Also this is monthly data so this corresponds to yearly seasonality.

FEEDBACK: ONLY ONE ACF, NOT BOTH
FEEDBACK: THIS IS WRONG. YOU CAN HAVE MONTHLY DATA WITH NO SEASONALITY. 
SEE SOLUTION
------
...
FEEDBACK: YOU DID NOT ADJUST ANYTHING
...
#2
unemp <- unemp %>% mutate(log_T = log(unemp))
stl_adjust <- unemp %>%
        model(stl = STL(log_T)) %>%
        components()
...
FEEDBACK: THESE ARE NOT AT ALL THE MODELS I REQUEST
...
FEEDBACK: NO GRAPH. AND WRONG MODEL
...
FEEDBACK: WRONG CODE: COPY PASTE WITHOUT PROPERLY ADAPTING
...
This is not white noise, but the variance of the remainder is smaller than the variance of the seasonal component.
------