---
title: "ZZ_TSA_ATamayo_2023Fall_Midterm"
editor: source
params:
  print_sol: false
toc: true
...
FEEDBACK: this is wrong. You are using a 7-MA and should use a 2x12-MA.
...
unemp %>%
 mutate(
  ...
  ) %>% autoplot(unemp) + geom_line(aes(y = `trend_class`), colour = "#D55E00")
```
...
unemp$detrended_class = classical_dcmp$det
unemp %>% autoplot(detrended_class) + scale_x_yearmonth(breaks = '1 year')
```
...
The length of the seasonal component I believe is six months. This is clearly visible in the ACF plot for the detrended component

FEEDABCK: wrong. The length of the seasonal period is 12 months. Positive spikes every 12 lags. See the theory sessions.
...
stl_adjust <- 
  unemp %>% 
    model(
      stl = STL(unemp ~ season(period = 7, window = 5) + 
                                trend(window = 7))
    ) %>% 
  components()
...
2. `decomposition_model()` using a **naive model** model for the *seasonally adjusted series* and a **seasonal naïve model** for the *seasonal component*. Call this model `dcmp_stl`.
  * For the **stl decomposition, adjust the parameters as you did in exercise 3.**
...
fit <- unemp %>% model(
  dcmp_drift<- decomposition_model(
      STL(unemp ~ season(period = 7, window = 5) + 
                                trend(window = 7)),
...
# FEEDBACK: no analysis performed, no points
```
