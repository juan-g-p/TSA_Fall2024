...
ts <- 
  ts %>% 
  mutate(
    bc_value = box_cox(value, lambda),
    d12_bc_value = difference(bc_value, 12),
    d12_d1_bc_value = difference(d12_bc_value, 1),
    d12_d1_d1_bc_value = difference(d12_d1_bc_value, 1)
  )

ts %>% autoplot(d12_bc_value)
ts %>% autoplot(d12_d1_bc_value)
ts %>% autoplot(d12_d1_d1_bc_value)

ts %>% ACF(d12_bc_value, lag_max = 48) %>% autoplot()
ts %>% ACF(d12_d1_bc_value, lag_max = 48) %>% autoplot()
ts %>% ACF(d12_d1_d1_bc_value, lag_max = 48) %>% autoplot()
```
...
```{r}
ts %>% gg_tsdisplay(d12_d1_bc_value, plot_type = "partial", lag_max = 48)
```

------------------------------------------------------------------------

Non-seasonal part
- ACF cut off at lag 1 -> q=1, p=0
- PACF cut off at lag 1 -> q=0, p=1

Seasonal part
- ACF cut-off at slag 1 -> Q=1, P=0
- PACF cut-off at slag 1 -> P=1, Q=0
...
fit <- 
  ...
  model(
    ...
    auto_arima = ARIMA(box_cox(value, lambda), stepwise=TRUE),
    auto_ETS = ETS(value)
  )

# Examine auto-arima model
fit %>% select(auto_arima) %>% report()

# Examine auto-ETS model
