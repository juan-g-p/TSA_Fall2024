...
1.  Use `group_by()` + `summarize()` to get the `mean()` and `median()` value of `arr_delay` each month. You will need to specify a correct value for the argument `na.rm` of the funtions `mean()` and ``median()`. 
