...
* These windows **differ on two points**.
...
Take the dataset `vic_elec` and aggregate the data to obtain the mean quarterly half hourly demand using `index_by()`. That each, for each quarter, compute the mean half-hourly demand.
